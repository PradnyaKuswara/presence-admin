...
import { School, Search, Users, ChevronRight, Layers } from 'lucide-react';
import { SchoolTenant, mockClasses } from '../../data/mockSaaSData';

interface ClassSchoolGridProps {
...
export const ClassSchoolGrid: React.FC<ClassSchoolGridProps> = ({
  schools,
  schoolSearchTerm,
  onSearchChange,
  onSelectSchool,
  ...
  return (
    <div className="space-y-4">
      <Card className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 shadow-xs">
        <div>
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            ...
            Langkah 1: Pilih Sekolah untuk Mengelola Master Kelas
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Pilih kartu sekolah di bawah ini untuk mengelola rombel (rombongan belajar) & wali kelas spesifik sekolah tersebut.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
        {schools.map((sch) => {
          const schoolClassCount = mockClasses.filter((c) => c.schoolId === sch.id).length;
            ...
            >
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 my-3 p-3 rounded-md bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs">
                  <div>
                    <p className="text-slate-400 text-[10px] uppercase font-semibold">Total Rombel Kelas</p>
                    <p className="font-bold text-blue-600 dark:text-blue-400 text-sm flex items-center gap-1">
                      <Layers className="w-3.5 h-3.5 text-blue-600" />
                      {schoolClassCount > 0 ? `${schoolClassCount} Rombel` : `${sch.totalClasses} Kelas`}
                    </p>
                  </div>
                  <div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                ...
                <button className="text-xs font-semibold text-blue-600 dark:text-blue-400 group-hover:translate-x-1 transition-transform flex items-center gap-1">
                  Kelola Master Kelas <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </Card>
          );
